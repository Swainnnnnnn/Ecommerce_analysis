...
@Controller
public class BrandHit {
    @Autowired //2. 注入一个jdbcTemplate，完成SQL 增删改查
    private JdbcTemplate jdbcTemplate;

    @GetMapping("/hit-brand") //1. URL映射：映射URL地址 -> getWeekReport
    @ResponseBody //4. 返回JSON数据
    public List<Map<String, Object>> getActThreeDayUser() {
        // 3. 使用JDBC template 执行一段SQL并返回数据
        return jdbcTemplate.queryForList("SELECT * FROM dws_uv_brand_total order by total desc limit 10");
        // 把数据返回
    }
}
